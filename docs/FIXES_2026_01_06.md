# Deformable DETR 训练脚本修复总结（2026-01-06）
...
#### 2. `train_detr_optimized.py` - 缺少 Deformable DETR 标准归一化 ✅
**问题**：图像只除以 255，未做 ImageNet mean/std 归一化  
**修复**：添加 Deformable DETR 标准归一化
```python
...
**修复**：使用官方 `DeformableDetrImageProcessor.post_process_object_detection`  
**位置**：line 206, line 216

#### 5. `train_detr_torchvision.py` - 接口不匹配（致命） ✅
**问题**：构建 transformers Deformable DETR 但用 torchvision 接口调用  
**修复**：标记为 `.BROKEN`  
...
#### 12. **低**：DeformableDetrImageProcessor 离线问题优化 ✅
...
### 3. `train_detr_optimized.py` - 标签格式错误
**问题**：bbox 是像素 xyxy 格式，未转换为 Deformable DETR 要求的归一化 cxcywh  
**修复**：在 `_resize()` 中转换
```python
# xyxy 像素 -> cxcywh 归一化
...
**问题**：
- 构建 `DeformableDetrForObjectDetection`（transformers）
...
**问题**：用 torchvision 风格调用 transformers Deformable DETR  
**修复**：标记为 `.BROKEN`  
...
### 1. 完整的 Deformable DETR 数据流水线
```
原始图像 (H_orig, W_orig)
  ↓ torchvision.io.read_image (C++ 解码)
  ↓ Deformable DETR 归一化 (ImageNet mean/std)
...
| 标签 | 归一化 | cxcywh | Deformable DETR 训练格式 |
| 预测 | 归一化 | cxcywh | Deformable DETR 输出 |
...
4. **torchvision Deformable DETR 未来**
   - 等 CUDA 环境 + torchvision 包含 Deformable DETR
   - 再开发纯 torchvision 版本
   - 保持与当前脚本相同的数据处理逻辑
