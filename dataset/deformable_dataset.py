...
class DeformableCOCODataset(torch.utils.data.Dataset):
    ...
    def __init__(self, img_folder, ann_file, transforms=None, return_masks=False, image_id_offset=0):
        """
        Args:
            img_folder: 图像文件夹路径
...
def build_deformable_dataloader(config, image_set='train'):
    ...
    if 'datasets' in dataset_config:
        ...
        for ds_conf in dataset_config['datasets']:
            ...
            ann_file = str(root / ds_conf[f'{image_set}_ann'])
            ...
            image_id_offset += (max(ds.ids) + 1) if ds.ids else 0
            print(f"📦 已添加数据集: {ds_conf.get('name', 'unnamed')} ({len(ds)} 样本)")
